...
static int power_value_fd;
static int power_direction_fd;
...
* Function:    powerWrite()
*
* Description: Write to the power pin
*
* Parameters:  value - outpur value (HIGF|LOW)
*
********************************************************************/
static void powerWrite(PIN_STATE_t value)
{
	int fd = power_value_fd;
	char d[1];
	//printf("power_value_fd: %d\n", power_value_fd);
	d[0] = (value == LOW ? '0' : '1');
...
#if DEBUG
	printf("WARNING: clock resolution (%uns) is not good enough on you system\n", (unsigned int)res_ts.tv_nsec); // debug info
#endif
  }
  else
  {
#if DEBUG
	  printf("Clock resolution is (%uns): Ok \n", (unsigned int)res_ts.tv_nsec);
#endif
  }
}
...
void dhtSetup_gpio(uint8_t pin, DHT_MODEL_t model)
{
  ...
  printf("Open gpio direction file\n");
  ...
  if (fd < 0) {
    fprintf(stderr, "Open %s: %s\n", b, strerror(errno));
    error_code = ERROR_OTHER;
	printf("Error Open gpio direction file\n");
    return;
  }
  value_fd=fd;
  printf("value_fd: %d\n", value_fd);
  ...
  if ( model == AM2302 || model == RHT03) {
     sensor_model = DHT22;
	 printf("( model = AM2302 || model = RHT03)\n");
  }   
  else if ( model == AUTO_DETECT) {
    sensor_model = DHT22;
	printf("model = AUTO_DETECT\n");
    readSensor();
    if ( error_code == ERROR_TIMEOUT ) {
      sensor_model = DHT11;
	  printf("Warning: in case we auto detect a DHT11\n");
  ...
  printf("sensor_model: %d\n", sensor_model);
  error_code = ERROR_NONE;
  printf("End Init dhtSetup_gpio\n");
}
...
*
********************************************************************/
void dhtSetup_Power_gpio(uint8_t pin)
{
	int fd;
	char b[64];
	...
	// store globals
	power_data_pin = pin;
	printf("Init dhtSetup_Power_gpio\n");
	resetTimer(); // Make sure we do read the sensor in the next readSensor()
	...
	power_direction_fd = fd;
	printf("power_direction_fd: %d\n", power_direction_fd);
	// Open gpio value file for fast reading/writing when requested
	sysfs_filename(b, sizeof(b), pin, "value");
	fd = open(b, O_RDWR);
	if (fd < 0) {
	...
	printf("power_value_fd: %d\n", power_value_fd);
	error_code = ERROR_NONE;
	printf("End Init dhtSetup_Power_gpio\n");
}
...
void readSensor_gpio()
{
  ...
  float temp = 0;
#if DEBUG
  long t1, t2, t3, t4, t2_t1, t3_t2, t4_t3; // debug info
#endif
 ...
 //  }
  powerWrite(HIGH); // Init
  usleep(500);
  powerWrite(LOW); // LOW HIGH
  usleep(500);
  powerWrite(HIGH); // Send start signal
  t1 = micros();
...
//      if ( age > MAX_BIT_LENGTH ) { // max 120 (70 uS)
...
//	Humidity_AM2301 = rawHumidity >> 8;// humidity = rawHumidity >> 8;
//    temperature = rawTemperature >> 8;
//  }
//  else {
//	  temp = rawHumidity;
//	  temp = temp / 10;
//	  //humidity = temp * 0.1; // del - error compille
//	  Humidity_AM2301 = temp;
//    if ( rawTemperature & 0x8000 ) {
//      rawTemperature = -(int16_t)(rawTemperature & 0x7FFF);
//    }
//    // temperature = ((int16_t)rawTemperature) * 0.1;  // del - error compille
//	temp = rawTemperature / 10;
//	temperature = temp;
//  }
