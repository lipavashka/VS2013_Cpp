...
/* Global variables */
float temperature;
float Humidity_AM2301; 
// float humidity;
uint8_t data_pin;
uint8_t power_data_pin;
...
extern void dhtSetup_Power_gpio(uint8_t pin);
 ...
 * 
 ********************************************************************/
void dhtSetup(uint8_t power_pin, uint8_t pin, DHT_MODEL_t model)
{
	if (pin)  {
		unExport_GPIOPin("25");
		unExport_GPIOPin("38");
		dhtSetup_Power_gpio(power_pin);
...
float getHumidity()
{
	return Humidity_AM2301; // humidity;
}
...
void readSensor()
{
	if (data_pin)
	{
		printf("readSensor_gpio\n");
		readSensor_gpio();
	}
	else
	{
		printf("readSensor_spi\n");
		readSensor_spi();
	}
}
