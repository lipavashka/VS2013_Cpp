...
pthread_t bmp085_thread;  // bmp085 thread
...
void unExport_GPIOPin(char* pin)
{
	...
	// unExport chosen pin
	write(fd, pin, strlen(pin)); // unExport GPIO pin
	close(fd);
}
...
void setMux(void)
{
	...
	setGPIOPin("54", "out", "strong", "1"); // SPI1_MISO (Pin_12 GPIO38)
}
...
volatile int thread_arg = 0; // nothing doing, only for disapp. warning

void* do_bmp085_thread(void *arg)
{
	if (arg == NULL){
		thread_arg = 0;
	}
	...
	while (1){
		if (pthread_equal(id, bmp085_thread))
		{
